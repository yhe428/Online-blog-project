...
async function retrieveAllSearchResults(input) {
    ...
    try {
        ...
        const resultsArray = await db.all(SQL`select distinct articleId, categoryName, title, content, authorfName, authorlName, articleDate, imageName, imageHeight, avatar
        from ArticlesSearch
